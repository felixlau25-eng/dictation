...
function inspectEngine(lang, pref) {
  ...
  if (lang === "cantonese") {
    ...
    return { label: "\u96f2\u7aef\u9999\u6e2f\u7cb5\u8a9e", detail: "\u7cb5\u8a9e\u5831\u8b80 \u00b7 \u5514\u6703\u6539\u7528\u666e\u901a\u8a71", source: "cloud" };
  }
...
function playSrc(src, rate, gen) {
  return new Promise((resolve, reject) => {
    ...
    audio.play().then(function () {
      ...
      audio.style.cssText = "position:fixed;left:12px;right:12px;bottom:12px;width:auto;z-index:99;height:40px";
    });
  });
}
...
async function speakText(text, lang, rate, enginePref) {
  ...
  const gen = ++speakGen;
  await unlockAudio();
  const forceSystem = enginePref === "system" && !!pickVoice(lang);
  if (!forceSystem) {
    try { await speakCloud(value, lang, rate, gen); return; }
    catch (e) {
      if (lang === "cantonese" && !pickVoice("cantonese")) throw e;
    }
  }
